...
- Source builds can now use the clang-cl compiler and append custom extra CMake flags (e.g. -DGGML_NATIVE=OFF, AVX512 tuning) from the Advanced section.
...
### Fixed
- Decimal backend parameters such as Repeat Penalty now accept and increment fractional values without browser validation errors.

## [1.5.2] - 2026-07-20

### Fixed
- Profile migration now rebases saved LiteLLM config paths into `%APPDATA%\llamadeck`, preserves the real migrated config, removes the partial legacy directory that version 1.5.1 could recreate, and records migration completion so a later stale directory cannot replace the canonical profile.
