...
## Completed
- Hardened the one-time profile migration after version 1.5.1 exposed an embedded absolute LiteLLM config path. Startup now rebases that path into `%APPDATA%\llamadeck`, preserves the migrated config, removes a legacy directory only when it contains residual generated files, and writes a completion marker so a later stale directory can never displace the canonical profile.
- Replaced the permanent legacy-profile fallback with a one-time packaged migration into `%APPDATA%\llamadeck`. The canonical profile is moved without copying large model/backend folders, stale CLI endpoint and lock files are removed, and an existing LlamaDeck profile is preserved under a timestamped LlamaDeck backup name. The CLI now uses only the current profile and rejects descriptors whose process is dead; renderer preferences similarly move to `llamadeck_*` keys and delete their legacy duplicates after first read.
- Expanded the PowerShell CLI into an agent-focused control surface with strict JSON template create/update/delete/validation, bounded in-memory log retrieval and NDJSON follow mode, model readiness waiting through `/health`, backend listing/selection, and machine-readable capabilities. CLI mutations broadcast template/backend changes so an open GUI refreshes immediately; template writes are atomic and destructive deletion requires `--yes`.
- Expanded the same CLI with app-managed LiteLLM status, lifecycle, install/update, connectivity test, model listing, cursor-based logs, and validated YAML config commands. LiteLLM CLI mutations refresh the open GUI, config writes remain atomic, and saved or literal API keys are redacted from every CLI output surface.
- Added an installed Windows PowerShell CLI (`llamadeck`) with JSON-returning `template get/list/start/stop`, `status`, `app show`, and version/help commands. The client auto-starts LlamaDeck when needed, uses a per-launch authentication token plus a per-user named pipe rather than a TCP listener, and is added to/removed from the user `PATH` by the NSIS installer. GUI and CLI launches now share the same argument builder and main-process launch path, while the active backend selection is persisted in main-process settings so unpinned templates resolve consistently from either surface.
- Implemented smooth in-app updates for LlamaDeck itself via `electron-updater`. LlamaDeck now checks GitHub Releases for a newer version on launch (configurable in Settings), downloads updates in the background, and prompts the user to restart at their convenience. The NSIS installer path is locked (`allowToChangeInstallationDirectory: false`) so Windows taskbar pins and Start-menu shortcuts survive updates. A new Settings → App Updates section surfaces current version, available update, download progress, release notes, and `Check on launch` / `Auto-download` toggles. Existing users accept a one-time repin on the next reinstall; new installs use `%LOCALAPPDATA%\Programs\llamadeck` going forward.
- Fixed duplicate-instance launch on Windows. LlamaDeck now acquires `app.requestSingleInstanceLock()` at startup and quits any subsequent launches. The `second-instance` event focuses the existing main window (restoring from minimized and showing from hidden) so clicking the taskbar icon while the app is already running brings it to front instead of starting a new process.
  - `src/main/updateManager.ts` — wraps `electron-updater` events into an `UpdateState` and broadcasts via `broadcastToRenderer`. Reuses the extracted `hasActiveWork()` guard so updates never race with llama.cpp source builds, model downloads, or running models.
  - `src/main/updateSettings.ts` — atomic-read/write of `userData/update-settings.json` (Zod-validated).
  - `src/main/activeWork.ts` — extracted `hasActiveTransfers()` so both `updateManager` and the existing IPC handler call sites share one function.
  - `src/shared/update.ts` — `UpdateState`, `UpdatePreferences`, `UpdateProgress`, `UpdateInfo` types and Zod schemas.
  - `src/main/ipc.ts` — `update:get-state`, `update:check`, `update:download`, `update:install-and-restart`, `update:get-preferences`, `update:set-preferences` handlers.
  - `src/preload/index.ts` — `window.api.update*` and `onUpdateStateChanged`.
  - `src/renderer/src/components/UpdateSettings.tsx` — pure presentation component for the Settings → App Updates section.
  - `electron-builder.yml` — locked install path; added `publish` block pointing at `afatihakpolat/llamadeck`.
  - Spec at `docs/specs/smooth-update-path/`.
...
## Verification
- `npm run test:run` (164 tests passing, including template and LiteLLM config CLI ingestion, validation exit semantics, redaction, command dispatch, authenticated named-pipe requests, and PowerShell round trips)
- `npm run build` after the agent-focused CLI expansion and GUI synchronization changes
- `npm run test:run` (148 tests passing, including authenticated named-pipe requests and a real PowerShell client round trip)
- `npm run build`
- `npm run package` (x64/arm64 zips and combined NSIS installer; packaged CLI resources and installer PATH hook verified)
...
- `npm run test:run` (13 files, 127 tests) and `npm run build` after adding the in-app updater (`electron-updater`), the locked NSIS install path, the `updateSettings` persistence, the `activeWork` extraction, and the Settings → App Updates UI

## Next Recommended Check
- Manual smoke test for the in-app updater: install the current packaged build via NSIS, pin to taskbar, trigger a manual `workflow_dispatch` build that bumps the patch version and publishes a GitHub Release, then in Settings → App Updates click "Check for updates" and confirm the new version is detected. Click "Download", confirm progress reaches 100%, click "Restart to install", confirm the app relaunches into the new version with the same taskbar pin and Start-menu shortcut.
...
- Rename caveat: packaged upgrades preserve the legacy Hexllama data directory, but the installer identity, taskbar pins, and shortcuts still move to the new LlamaDeck identity rather than migrating in place. **Resolved as of [Unreleased]**: the NSIS install path is now locked (`allowToChangeInstallationDirectory: false`) and the in-app updater writes into the same `%LOCALAPPDATA%\Programs\llamadeck` folder, so the taskbar pin survives future updates. Existing users on the previous open-ended installer accept a one-time repin.
