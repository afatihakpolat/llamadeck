...
6. (When ready) push to `main` on origin (`afatihakpolat/llamadeck` — the release repo; `upstream` points at the original `hexllama` and is not used for releases). The `Build Installers and Release` workflow (`.github/workflows/build.yml`) is triggered by the push (also `workflow_dispatch`), reads the version straight from `package.json`, creates a GitHub Release named after that version (no git tag is ever created), and uploads the `dist/` artifacts from the Windows and Linux runners. The in-app updater finds the release's `latest.yml` through the `publish` block in `electron-builder.yml`; missing intermediate releases do not matter — only the newest version needing to be higher than the running one.
